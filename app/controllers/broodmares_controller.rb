...
## Jay those are the same sequence of characters below here
# GIRO:  CHANGED BroodmaresController to BroodmaresController
#############################################
# 201702281633L     EL MARTES   GIRO
# It's all 'borrowed' from rails conventions!
...
class BroodmaresController < ApplicationController
  ...
  def new
    ...
    @stallion_options = Stallion.all.map do |stallion|
      [stallion.name, stallion.id]
    end
  end
  ...
  def edit
    @broodmare = Broodmare.find(params[:id])
    # you probably want this for your form. indent it properly though!
    @stallion_options = Stallion.all.map do |stallion|
      [stallion.name, stallion.id]
    end
  ...
  def destroy
    @broodmare = Broodmare.find(params[:id])
    # I'd remove this comment below! kill the clutter!
